...
The core problem this work addresses is the \emph{disconnect between how AI ethics is taught and how it is actually produced in technical practice}. Specifically, three interrelated problems motivate RewardCraft:

\begin{enumerate}
  \item \textbf{Passive AI education.} Most K--12 AI tools position students as \emph{consumers} of pre-trained models. Students observe AI behavior but do not control the value choices that produce it, leaving the connection between design decisions and ethical outcomes invisible.
  \item \textbf{Absent reward-design literacy.} Reinforcement learning---the paradigm underlying recommendation engines, game AI, robotics, and language model fine-tuning---is rarely taught before college, and when it is, instruction focuses on algorithm mechanics rather than on \emph{objective specification} as a socio-technical decision.
  \item \textbf{Ethics as an add-on.} AI ethics curricula typically introduce bias, fairness, and harm as separate topics, after the technical work is done. This separation obscures the central insight that ethical failures in deployed AI systems are often the \emph{correct and intended outcome} of a misspecified reward or objective function---not a bug, but a design flaw.
\end{enumerate}

\noindent These three problems combine to produce high school graduates who can \emph{use} AI tools but cannot reason about \emph{why} those tools behave the way they do or \emph{how} to change that behavior responsibly.
...
\section{Related Work: Existing Solutions and Their Limitations}
Several lines of prior work address parts of the problem described above. We review each and identify the residual gap that motivates RewardCraft.
...
RewardCraft builds on this tradition but differs in two design priorities: (i) \emph{reward function design} is the primary learner-controlled artifact (not merely a parameter), and (ii) the system explicitly uses \emph{specification gaming} as an instructional mechanism to bridge RL concepts and alignment/ethics. Table~\ref{tab:compare} summarizes key distinctions across comparable tools.

\begin{table*}[t]
\caption{Feature comparison of RewardCraft against related educational RL tools.}
\label{tab:compare}
\centering
\small
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Feature} & \textbf{RewardCraft} & \textbf{ARtonomous} \cite{dietz2022} & \textbf{TrainYourSnakeAI} \cite{hinojosa2025} & \textbf{PlayGrid} \cite{quiloan2023} \\
\midrule
Target age group              & 14--18 (HS)     & 11--14 (MS)       & 11--14 (MS)       & 12--18 (MS/HS)  \\
RL algorithm exposed          & Tabular Q-learn & Q-learning (AR)   & Q-learning        & Q-learning      \\
Reward function design        & \textbf{Primary artifact} & Fixed reward    & Preset only       & Partial sliders \\
Specification gaming pedagogy & \textbf{Yes}    & No                & No                & No              \\
Real-time Q-table heatmap     & \textbf{Yes}    & No                & No                & Limited         \\
Post-training reflection tool & \textbf{Yes}    & No                & No                & No              \\
Research data logging         & \textbf{SQLite + CSV}& No           & No                & No              \\
Guided onboarding tutorial    & \textbf{Yes (8 slides)} & Facilitator & Facilitator       & Partial         \\
\bottomrule
\end{tabular}
\end{table*}
...
\subsection{The Gap: Why Existing Solutions Are Insufficient}
Taken together, prior tools address RL education and game-based learning but leave three critical gaps that RewardCraft is designed to fill:

\begin{enumerate}
  \item \textbf{Reward design is not the primary artifact.} In ARtonomous, TrainYourSnakeAI, and PlayGrid, reward functions are either fixed by the system or only partially exposed (e.g., preset selection, minor sliders). Students observe learning but do not \emph{author} the objective. RewardCraft makes the reward function the student's central creative act.
  \item \textbf{Specification gaming is not intentionally produced.} No existing tool in our review deliberately engineers conditions where the agent succeeds at maximizing reward while failing the intended goal. This is the most concrete and memorable way to teach AI alignment, yet it is absent from current K--12 tooling.
  \item \textbf{No structured reflection or data collection.} Existing tools end when training ends. RewardCraft closes the learning loop with embedded post-training reflection prompts and a backend research database that logs student responses, reward configurations, and episode outcomes---enabling both pedagogical assessment and empirical research on learning.
\end{enumerate}

\noindent Table~\ref{tab:compare} quantifies these distinctions across comparable tools.
...
\caption{Specification gaming trace across three preset reward configurations (300 training episodes each). Panel~(a) shows smoothed episode reward; panel~(b) shows smoothed win rate. The \emph{Gold Hoarder} preset (penalizes spending, rewards saving) accumulates 10$\times$ higher episode reward than \emph{Balanced} yet achieves a near-zero win rate---the agent optimizes the measurable reward signal while failing the intended objective. \emph{Balanced} and \emph{Greedy Killer} converge to similar win rates, but with qualitatively different learned policies visible in Q-table analysis.}
...
\subsection{Student Feedback Loop}
Closing the learning cycle requires more than observation---students must be prompted to articulate what they saw and connect it to broader implications. RewardCraft implements a structured \emph{feedback loop} with three components:

\begin{enumerate}
  \item \textbf{In-game reward breakdown panel.} During training, a real-time panel itemizes every reward event (e.g., ``+10 enemy defeated,'' ``$-$50 enemy reached base'') at each step, making the agent's incentive structure transparent rather than implied.
  \item \textbf{Post-training reflection modal.} When training completes, a modal presents two structured prompts: (a) \emph{``Did your agent do anything unexpected? Describe any unintended behaviors.''} and (b) \emph{``Can you think of a real-world AI system where a similar reward mismatch might cause problems?''} These prompts are aligned with RQ2 and RQ3 (see Section~\ref{sec:eval}) and encourage transfer from the game context to real AI systems.
  \item \textbf{Persistent research database.} Student names, reward configurations, training outcomes, and reflection responses are logged to a server-side SQLite database and are exportable as CSV files via REST endpoints (\texttt{/api/research/reflections/csv}, \texttt{/api/research/sessions/csv}). This enables instructors to review responses after class and provides the data infrastructure for the planned classroom study.
\end{enumerate}

\noindent This feedback loop is absent from all comparable tools reviewed in Section~2 and is a key differentiating contribution of RewardCraft.
...
The state abstraction is designed to be small enough for rapid learning but rich enough to produce meaningful trade-offs. The current implementation discretizes the game into 486 states derived from six factors (Table~\ref{tab:state}): gold availability, enemy presence, tower count, wave progress, threat level from enemy composition, and whether a slow tower is deployed. This yields a tractable Q-table that students can inspect and discuss.
...
\midrule
Gold level & 0, 1, 2 & Resource constraint \\
Enemies on field & 0, 1, 2 & Enemy presence \\
Towers built & 0, 1, 2 & Base defense \\
Wave progress & 0, 1, 2 & Episode stage \\
Threat level & 0, 1, 2 & Current pressure \\
Has slow tower & 0, 1 & Utility coverage \\
\midrule
...
\caption{Q-table heatmap after 300 training episodes (\emph{Balanced} preset). Rows represent the 30 most-visited game states (labeled by their six feature values: Gold, Enemies, Towers, Wave, Threat, Slow-tower). Columns represent the 12 available actions. Green cells indicate high Q-values (preferred actions); yellow cells indicate near-zero values (rarely visited state-action pairs). Stars~(\textbf{\large$\star$}) mark the best action per state. The heatmap shows that in no-enemy early-game states the agent has learned to \emph{Save} gold rather than build---a rational but potentially misaligned strategy students are asked to critique.}
\label{fig:qheat}
\end{figure}
